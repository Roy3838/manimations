...
# Calculate force between three bodies
        ...
        def twoDplanets():
            ...
            ).shift(5*UP+ LEFT*7).scale(1.1)

            text = Tex(r"3 Líneas", r" de Código!").shift(6*UP+RIGHT*5).scale(3).set_color(RED)
            text[1].shift(DOWN*2 + LEFT*6)


            text.shift(6*RIGHT)
            example_listing.shift(6*RIGHT)
            ...
            self.play(Write(example_listing),Write(text),self.camera.frame.animate.move_to(UP*2+ RIGHT*3))
